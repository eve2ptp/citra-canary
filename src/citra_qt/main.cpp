...
void GMainWindow::OnLoadState() {
    ...
    if (Settings::values.show_save_state_warning) {
        QMessageBox msg(this);
        msg.setIcon(QMessageBox::Icon::Warning);
        msg.setStandardButtons(QMessageBox::Ok | QMessageBox::Cancel);
        msg.setDefaultButton(QMessageBox::Ok);
        msg.setText(tr("Warning: Savestates are NOT a replacement for in-game saves, "
                       "and are not meant to be reliable.\n\nUse at your own risk!"));
        const int ret = msg.exec();
        if (ret == QMessageBox::Cancel) {
            return;
        }
        Settings::values.show_save_state_warning = false;
        config->Save();
    }
