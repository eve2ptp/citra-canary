...
void FragmentModule::WriteDepth() {
    ...
    const auto float24_round = [this](Id variable_id) {
        ...
        const Id bits{OpBitcast(u32_id, variable_id)};
        const Id rounded{OpBitwiseAnd(u32_id, variable_id, ConstU32(~0x7Fu))};
        return OpBitcast(f32_id, rounded);
    };
    const Id rounded_depth{float24_round(depth)};
    OpStore(gl_frag_depth_id, rounded_depth);
}
