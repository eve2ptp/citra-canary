...
void TextureRuntime::Reset() {
    scheduler.Finish();
    for (const auto& [tag, alloc] : allocation_map) {
        const auto& images = alloc.images;
        if (images[2].handle) {
...
Allocation TextureRuntime::Allocate(const VideoCore::SurfaceParams& params,
    ...
    if (auto it = allocation_map.find(tag); it != allocation_map.end()) {
...
void TextureRuntime::Recycle(const HostTextureTag tag, Allocation&& alloc) {
    allocation_map.emplace(tag, std::move(alloc));
}
...
void Surface::Download(const VideoCore::BufferTextureCopy& download,
    ...
    if (type == VideoCore::SurfaceType::DepthStencil) {
        LOG_INFO(Render_Vulkan, "Unsupported depth stencil download!");
        return;
    }
