...
SurfaceId RasterizerCache<T>::FindMatch(const SurfaceParams& params, ScaleMatch match_scale_type,
    ...
    ForEachSurfaceInRegion(params.addr, params.size, [&](SurfaceId surface_id, Surface& surface) {
        ...
        IsMatch_Helper(std::integral_constant<MatchFlags, MatchFlags::Reinterpret>{}, [&] {
            return std::make_pair(surface.CanReinterpret(params), surface.GetInterval());
        });
...
bool RasterizerCache<T>::ValidateByReinterpretation(Surface& surface, SurfaceParams params,
    ...
    if (reinterpret_id) {
        ...
        if (!boost::icl::length(copy_interval)) {
            return false;
        }
        ...
        const auto src_rect = src_surface.GetSubRect(copy_params);
        const auto dst_rect = surface.GetSubRect(copy_params);
        const TextureBlit reinterpret = {
            .src_level = src_surface.LevelOf(addr),
            .dst_level = surface.LevelOf(addr),
            .src_rect = src_rect * src_surface.res_scale,
            .dst_rect = dst_rect * surface.res_scale,
        };
        const bool rect_match = src_rect.GetWidth() == dst_rect.GetWidth() &&
                                src_rect.GetHeight() == dst_rect.GetHeight();
        const bool size_match =
            src_surface.width == surface.width && src_surface.height == surface.height;
        if (rect_match && size_match) {
            return runtime.Reinterpret(src_surface, surface, reinterpret);
        } else if (!size_match) {
            if (src_format != dst_format && is_gpu_modified) {
                LOG_DEBUG(HW_GPU,
                          "Attempted dimentional reinterpretation with different format, ignoring");
                return true;
            }
            LOG_WARNING(HW_GPU, "Unimplemented dimentional reinterpretatation {}x{} -> {}x{}",
