...
public:
    ...
    Frontend::Frame* GetPresentFrame() override {
        std::unique_lock<std::mutex> lock(swap_chain_lock);
        // wait for new entries in the present_queue
        present_cv.wait(lock, [&] { return !present_queue.empty(); });
        if (present_queue.empty()) {
            return nullptr;
        }
...
void RendererOpenGL::SwapBuffers() {
    ...
    if (video_dumping_shutdown_requested.exchange(false)) {
        // In this case, we still render the current frame, so that frame_dumper won't
        // be stuck waiting for present frame
...
void RendererOpenGL::CleanupVideoDumping() {
    video_dumping_shutdown_requested.store(true, std::memory_order_relaxed);
}
